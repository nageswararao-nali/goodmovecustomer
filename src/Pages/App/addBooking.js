...
import { StyleSheet, View, TextInput, Text, Image, TouchableOpacity, RefreshControl, Button, Keyboard, ScrollView, AsyncStorage, KeyboardAvoidingView, ActivityIndicator } from 'react-native';
...
var time = [
    { label: "Fixed", value: 'Fixed' }, { label: "Flexible", value: 'Flexible' },
]
var today = new Date();
...
class AddBooking extends Component {
        ...
        await AsyncStorage.getItem('booking_details').then(async (bDetails) => {
            if(bDetails) {
                ...
                this.setState({
                    ...
                    pickup_time: moment(p_df + " " + booking_details.pickup_time.split(" ")[0]).format('YYYY-MM-DD HH:mm:ss'),
                    pick_flexibility: booking_details.pickuptime_flexiblity_status,
                    drop_address: booking_details.dropoff_location,
                    drop_time: moment(d_df + " " + booking_details.dropoff_time.split(" ")[0]).format('YYYY-MM-DD HH:mm:ss'),
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            //picktime : '',
            weight: '', weight_Err: '',
            weight_unit: 12, weight_unit_Err: '',
            pack_type: '', 
            pack_type_Err: '',
            length: '', length_Err: '',
            length_unit: '', length_unit_Err: '',
            breadth: '', breadth_Err: '',
            breadth_unit: '', breadth_unit_Err: '',
            height: '', height_Err: '',
            height_unit: '', height_unit_Err: '',
            dimension_unit_Err: '',
            commodity_type: '', commodity_type_Err: '',
            pickup_time: '', pick_time_Err: '',
            pickup_add: '', pickup_add_Err: '',
            pick_flexibility: 'Fixed', drop_flexibility: 'Fixed',
            pick_flex_time: '', pick_flex_time_Err: '',
            dropmindate: today,
            drop_time: '', drop_time_Err: '',
            drop_add: '', drop_add_Err: '',
            drop_flex_time: '', drop_flex_time_Err: '',
            photo: null, photo_Err: '',
            filename: '',
            receiver_name: '', receiver_name_Err: '',
            mobile: '', mobile_Err: '',
            instruction: '', instruction_Err: '',
            isPickVisible: false, isDropVisible: false,
            dbpackage_type: '',
            dbdimension_unit: '',
            dbweight_unit: '',
            dbvolumn_unit: '',
            ...
            pickLattitude: '',
            pickLongittude: '',
            commodity_type_name: '', commodity_type_name_Err: '',
            pick_city: '',
            pick_state: '',
            pick_pincode: '',
            pick_country: '',
    ...
    PickupDatePicker = (datetime) => {
        ...
        var currentDateTime = moment(new Date()).add(1, 'hour').format('YYYY-MM-DD HH:mm:ss');
        var currentDate = moment().format('YYYY-MM-DD HH:mm:ss');

        if (picktime < currentDateTime) {

            this.setState({ pickup_time: moment(new Date()).add(1, 'hour').format('YYYY-MM-DD HH:mm:ss') })
        } else {
            this.setState({ pickup_time: moment(datetime).format('YYYY-MM-DD HH:mm:ss') })
        }
    };
    hidePickupPicker = () => {
        this.setState({ isPickVisible: false });
    ...
    DropDatePicker = (datetime) => {
        ...
        var dropTime = moment(this.state.pickup_time).add(1, 'hour').format('YYYY-MM-DD HH:mm:ss');

        if (this.state.pickup_time >= drop) {
            this.setState({ drop_time: moment(this.state.pickup_time).add(1, 'hour').format('YYYY-MM-DD HH:mm:ss') });
        } else if (drop <= dropTime) {
            this.setState({ drop_time: moment(this.state.pickup_time).add(1, 'hour').format('YYYY-MM-DD HH:mm:ss') });
        } else if (this.state.pickup_time < dropTime) {
            this.setState({ drop_time: moment(datetime).format('YYYY-MM-DD HH:mm:ss') });
        } else {
            this.setState({ drop_time: moment(datetime).format('YYYY-MM-DD HH:mm:ss') });
        }
    };
    ...
    getCategory = async () => {
        ...
        var response = CategoryAction().then(function (responseJson) {

            if (responseJson.isError == false) {
                this.setState({ dbpackage_type: responseJson.result.packaging_types })
                this.setState({ dbweight_unit: responseJson.result.weight_unit_types })
                this.setState({ dbvolumn_unit: responseJson.result.volume_unit_types })
                this.setState({ dbdimension_unit: responseJson.result.dimension_unit_types })
                this.setState({ dblenght_unit: responseJson.result.length_unit_types })
                this.setState({ dbheight_unit: responseJson.result.height_unit_types })
                this.setState({ dbbreadth_unit: responseJson.result.breadth_unit_types })
                this.setState({ dbcommodity_type_types: responseJson.result.commodity_type_types })
    ...
    handleChoosePhoto = async () => {
        ...
        if (Photoresult.cancelled != true) {
            ...
            this.setState({ filename: Photoresult.uri.substring(Photoresult.uri.lastIndexOf('/') + 1) })
    ...
    /** add booking */
    async submit() {
        const { weight, weight_unit, packaging_type, volumn, volumn_unit, length, length_unit, breadth, breadth_unit, height, height_unit, commodity_type, pickup_time, pickup_address, pick_flex_time, drop_time, drop_address, drop_flex_time, photo, receiver_name, mobile, instruction, dimension_unit, commodity_type_name } = this.state;
        ...
        var constraints = {
            ...
            drop_address: {
                ...
                equality: {
                    ...
                    comparator: function (v1, v2) {
                        return JSON.stringify(v1.toLowerCase()) != JSON.stringify(v2.toLowerCase());
                    }
                }
            },
            /*photo: {
        ...
        if (this.state.packaging_type == 28 || this.state.packaging_type == 30 || this.state.packaging_type == 34) {
        ...
        if (this.state.packaging_type == 29 || this.state.packaging_type == 31 || this.state.packaging_type == 32 || this.state.packaging_type == 33) {
            this.setState({ volumn_Err: '' })
            this.setState({ volumn_unit_Err: '' })
            var constraints2 = {
                length: {
        ...
        if (this.state.commodity_type == 43) {
            ...
            if (result2) {
                if (result2.commodity_type_name) {
                    this.setState({ commodity_type_name_Err: result2.commodity_type_name })
                } else {
                    this.setState({ commodity_type_name_Err: '' })
                }
            }
        ...
        formData.append('pickuptime_flexiblity_status', this.state.pick_flexibility);
        ...
        formData.append('dropofftime_flexiblity_status', this.state.drop_flexibility);
        ...
        formData.append('receiver_name', this.state.receiver_name ? this.state.receiver_name : 'Prakash');
        // formData.append('receiver_contact', this.state.mobile);
        formData.append('receiver_contact', this.state.mobile ? this.state.mobile : '7010020749');
        formData.append('instruction', this.state.instruction);
        formData.append('pickup_lat', this.state.pickLattitude ? this.state.pickLattitude : 13.098312);
        formData.append('pickup_long', this.state.pickLongittude ? this.state.pickLongittude : 80.175435);
        ...
        formData.append('dropoff_lat', this.state.dropLattitude ? this.state.dropLattitude : 13.098312);
        formData.append('dropoff_long', this.state.dropLongittude? this.state.dropLongittude : 80.175435);
    ...
    async clearText() {
        this.setState({
            ...
            pack_type: '', pack_type_Err: '',
            length: '', length_Err: '',
            length_unit: '', length_unit_Err: '',
            breadth: '', breadth_Err: '',
            breadth_unit: '', breadth_unit_Err: '',
            volumn: '', volumn_Err: '',
            volumn_unit: '', volumn_unit_Err: '',
            height: '', height_Err: '',
            height_unit: '', height_unit_Err: '',
            commodity_type: '', commodity_type_Err: '',
            pickup_time: '', pickup_time_Err: '',
            pickup_add: '', pickup_add_Err: '',
            pick_flexibility: 'Fixed', drop_flexibility: 'Fixed',
            showpick: '', showdrop: '',
            pick_flex_time: '', pick_flex_time_Err: '',
            ...
            photoResult: '',
            commodity_type_name_Err: '', commodity_type_name: '',
            pickLattitude: '',
            pickLongittude: '',
            pick_city: '',
            pick_state: '',
    ...
    render() {
        ...
        for (let userObject of this.state.dbpackage_type) {
            packagTypes.push(<Picker.Item value={userObject.id} label={userObject.cate_name} key={userObject.id}></Picker.Item>)
        }

        const weightUnit = [];
        for (let userObject of this.state.dbweight_unit) {
            weightUnit.push(<Picker.Item value={userObject.id} label={userObject.cate_name} key={userObject.id}></Picker.Item>)
        }

        const volumnUnit = [];
        for (let userObject of this.state.dbvolumn_unit) {
            volumnUnit.push(<Picker.Item value={userObject.id} label={userObject.cate_name} key={userObject.id}></Picker.Item>)
        }

        const dimensionUnit = [];
        for (let userObject of this.state.dbdimension_unit) {
            dimensionUnit.push(<Picker.Item value={userObject.id} label={userObject.cate_name} key={userObject.id}></Picker.Item>)
        }

        const lenghtUnit = [];
        for (let userObject of this.state.dblenght_unit) {
            lenghtUnit.push(<Picker.Item value={userObject.id} label={userObject.cate_name} key={userObject.id}></Picker.Item>)
        }

        const heightUnit = [];
        for (let userObject of this.state.dbheight_unit) {
            heightUnit.push(<Picker.Item value={userObject.id} label={userObject.cate_name} key={userObject.id}></Picker.Item>)
        }

        const breadthUnit = [];
        for (let userObject of this.state.dbbreadth_unit) {
            breadthUnit.push(<Picker.Item value={userObject.id} label={userObject.cate_name} key={userObject.id}></Picker.Item>)
        }

        const commodity_type = [];

        for (let userObject of this.state.dbcommodity_type_types) {
            commodity_type.push(<Picker.Item value={userObject.id} label={userObject.cate_name} key={userObject.id}></Picker.Item>)
        }
						...
						<View style={styles.container}>
                            <View style={styles.form}>
                                <View style={styles.control}>
                                    ...
                                    {this.state.weight_Err ? <Text style={styles.error}>{this.state.weight_Err}</Text> : null}
                                    {this.state.weight_unit_Err ? <Text style={styles.error}>{this.state.weight_unit_Err}</Text> : null}
                                ...
                                */ }
                                {this.state.packaging_type == 28 || this.state.packaging_type == 30 || this.state.packaging_type == 34 ?
                                    (
                                        <View style={styles.control}>
                                            ...
                                            {this.state.volumn_Err ? <Text style={styles.error}>{this.state.volumn_Err}</Text> : null}
                                            {this.state.volumn_unit_Err ? <Text style={styles.error}>{this.state.volumn_unit_Err}</Text> : null}
                                        </View>
                                    )
                                    : null}

                                {this.state.packaging_type == 29 || this.state.packaging_type == 31 || this.state.packaging_type == 32 || this.state.packaging_type == 33 ?
                                    (
                                        (<View><View style={[styles.control, { flexDirection: 'row' }]}>
                                            <View style={{ width: "23%" }}>
                                            ...
                                            <View style={{ width: "23%" }}>
                                                ...
                                                <View style={styles.DimensionInput}>
                                                    <TextInput
                                                        ...
                                                        keyboardType="numeric"
                                                        placeholder="B"
                                                        onChangeText={(breadth) => this.setState({ breadth })}
                                            ...
                                            <View style={{ width: "30%" }}>
                                                ...
                                                <View style={{ borderWidth: 1, borderColor: "rgba(119,119,119,0.6)", borderRadius: 5, paddingTop: 5, paddingBottom: 5 }}>
                                                    <Picker
                                                        selectedValue={this.state.dimension_unit}
                                                        style={{ borderWidth: 1, borderColor: "blue", height: 29 }}
                                                        onValueChange={(itemValue, itemIndex) =>
                                                            this.setState({ height_unit: itemValue, breadth_unit: itemValue, length_unit: itemValue, dimension_unit: itemValue })
                                                        }>
                                            ...
                                            <View>
                                                {this.state.height_Err ? <Text style={styles.error}>{this.state.height_Err}</Text> : null}
                                                {this.state.breadth_Err ? <Text style={styles.error}>{this.state.breadth_Err}</Text> : null}
                                                {this.state.length_Err ? <Text style={styles.error}>{this.state.length_Err}</Text> : null}
                                                {this.state.dimension_unit_Err_Err ? <Text style={styles.error}>{this.state.dimension_unit_Err_Err}</Text> : null}
                                            </View>
                                        </View>
                                        )
                                    ) : null
                                ...
                                */ }
                                <View style={styles.control}>
                                    ...
                                    <Row style={styles.pickadd}> 
                                        <Col style={{ width: '6%',}}>
                                            <Image source={require('../../images/booking-list-icon/Pickup-50x50.png')}
                                                style={styles.icon}>
                                            </Image>
                                        </Col>
                                      
                                        <Col style={{width: '86%', paddingLeft: '2%' , paddingLeft: '1%' , paddingRight: '1%'}}>
                                            <TouchableOpacity 
                                            onPress={() => navigate('AddressPage', { address_type: 'Pickup' })}>   
                                                    <TextInput
                                        ...
                                        <Col style={{width: '8%',}}>
                                            <TouchableOpacity 
                                                onPress={() => navigate('AddressPage', { address_type: 'Pickup' })}>
                                                <Image source={require('../../images/Detact-Location.png')}
                                                    style={{ width: 20, height: 20, marginTop: 5 }}></Image>
                                            </TouchableOpacity>
                                        </Col>

                                    </Row>
                                    {this.state.pickup_add_Err ? <Text style={styles.error}>{this.state.pickup_add_Err}</Text> : null}
                                </View>
                                ...
                                <View style={styles.control}>
                                    ...
                                    <View style={styles.pickadd}>
                                        ...
                                        <Text style={{ alignItems: 'flex-end', paddingTop: 5, paddingRight: 5, color: '#777' }}>{this.state.showpick ? 'Flexible' : 'Fixed'}</Text>
                                    </View>
                                    ...
                                    <DateTimePicker
                                        ...
                                        is24Hour={true}
                                    />
                                    {this.state.pickup_time_Err ? <Text style={styles.error}>{this.state.pickup_time_Err}</Text> : null}
                                </View>
                                { /*
                                <View style={styles.control}>
                                ...
                                */ }
                                {this.state.showpick ?
                                    (
                                        <View style={styles.control}>
                                            <Text style={styles.label}>Select Pickup Time Flexibility</Text>
                                            <View style={styles.pickerPack}>
                                                <Picker
                                                    ...
                                                    <Picker.Item style={styles.label} label="- Select -"></Picker.Item>
                                                    <Picker.Item style={styles.label} label="30 minutes" value="30 minutes"></Picker.Item>
                                                    <Picker.Item style={styles.label} label="1 hours" value="1 hours"></Picker.Item>
                                                    <Picker.Item style={styles.label} label="2 hours" value="2 hours"></Picker.Item>
                                                </Picker>
                                            </View>
                                            {this.state.pick_flex_time_Err ? <Text style={styles.error}>{this.state.pick_flex_time_Err}</Text> : null}
                                        </View>
                                    )
                                    : null
                                }
                                ...
                                <View style={styles.control}>
                                    ...
                                    <Row style={styles.pickadd}>
                                        <Col style={{width: '6%',}}>
                                            <Image source={require('../../images/booking-list-icon/Location-50x50.png')}
                                                style={styles.icon}>
                                            </Image>
                                        </Col>
                                        <Col style={{width: '86%', paddingLeft: '2%'}}>
                                            <TouchableOpacity 
                                                onPress={() => navigate('AddressPage', { address_type: 'Drop-off' })}>
                                                    <TextInput
                                                        editable={false} selectTextOnFocus={false}
                                                        placeholder="Select address from map"
                                                        onChangeText={(drop_address) => this.setState({ drop_address })}
                                    ...
                                    {this.state.drop_add_Err ? <Text style={styles.error}>{this.state.drop_add_Err}</Text> : null}
                                </View>
                                ...
                                <View style={styles.control}>
                                    ...
                                    <View style={styles.pickadd}>
                                        ...
                                        <Text style={{ alignItems: 'flex-end', paddingTop: 5, paddingRight: 5, color: '#777' }}>{this.state.showdrop ? 'Flexible' : 'Fixed'}</Text>
                                    </View>
                                    ...
                                    />
                                    {this.state.drop_time_Err ? <Text style={styles.error}>{this.state.drop_time_Err}</Text> : null}
                                </View>
                                { /* 
                                <View style={styles.control}>
                                ...
                                */}
                                {this.state.showdrop ?
                                    (
                                        <View style={styles.control}>
                                            <Text style={styles.label}>Select Drop Time Flexibility</Text>
                                            <View style={styles.pickerPack}>
                                                <Picker
                                            ...
                                            {this.state.drop_flex_time_Err ? <Text style={styles.error}>{this.state.drop_flex_time_Err}</Text> : null}
                                        </View>
                                    )
                                    : null
                                }
