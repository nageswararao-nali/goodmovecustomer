...
class MyBooking extends Component {
  ...
  async getManagerAccountDetail() {
    ...
    managerPaymentOptionAction((accountActiondData = null), Token).then(
      (responseJson) => {
        if (responseJson.isError == false) {
          this.setState({
      ...
      paymentUploadAction(PaymentData, Token).then((responseJson) => {
        if (responseJson.isError == false) {
          ...
          this.sendNotification(
            responseJson.result.token,
            responseJson.result.booking_id,
            responseJson.result.customer_name,
            responseJson.result.transaction_id
          );
  ...
  renderItem = ({ item }) => {
    ...
    if (item.booking_canceled_by_manager == 1) {
      ...
      if (item.quotation_status == 0) {
        ...
        item.booking_accept_status == 1 &&
        item.quotation_payment_status == 0
      ) {
    ...
    return (
      <TouchableOpacity
        style={styles.card}
        onPress={() => {
          this.props.navigation.navigate("BookDetail", { itemId: item.id });
        }}
      >
        <View style={styles.ItemContainer}>
          ...
          <View style={styles.InnerContainer}>
            <View style={styles.innerleft}>
              ...
              <View style={styles.LabelContainer}>
                <Image
                  source={require("../../images/booking-list-icon/Qautation50x50.png")}
                  style={styles.iconsize}
                />
            ...
            <View style={styles.ActionIconContainer}>
              ...
              >
                <Image
                  source={require("../../images/booking-list-icon/Right-Arrow-20x20.png")}
                />
              </TouchableOpacity>
              {showPaymentButton == 1 ? (
                <View style={{ flexDirection: "row", marginTop: -13 }}>
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      this.opencanclePaymentModal(item.id);
                    }}
                    ...
                    <View>
                      ...
                      <View style={styles.ModalContentContainer}>
                        <Text style={styles.ContentText}>
                          Thanks for accepting the quotation. Please use the
                          account details provided here to make payment. Once
                          you have completed the payment, please provide
                          Transaction ID so we can verify the payment.
                        </Text>
                      </View>
                      ...
                      <View style={styles.combine}>
                        ...
                        <TextInput
                          ...
                          onChangeText={(transaction_id) =>
                            this.setState({ transaction_id })
                          }
                        ></TextInput>
                      </View>
                      {this.state.transaction_id_Err ? (
