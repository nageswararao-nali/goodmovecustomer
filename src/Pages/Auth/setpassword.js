...
import { StyleSheet, View, Image, Text, TextInput, TouchableOpacity, ScrollView, Keyboard, KeyboardAvoidingView, ActivityIndicator } from 'react-native';
...
export default class SetPassword extends Component {
	...
	async submit() {
		...
		const result = validate({ password: this.state.password, confirmpassword: this.state.confirmPassword }, constraints);
		/* Call validation library for validate mobile and password */
		...
		/* Validation check */
		if (!result && aValue != '' && bValue != '' && cValue != '' && dValue != '') {
			...
			/* Verify OTP */
			var response = verifyOTPAction(mobile,enteredOTP).then(function (responseJson) {
						...
						<View style={styles.container}>
							...
							<View style={styles.loginbox}>
								<View style={styles.loginlogo}>
									<Image source={require('../../images/New-Password.png')} style={styles.login}></Image>
								</View>
								<View style={styles.content}>
									<Text style={styles.intro}>Enter The 4-Digit Code Sent to Your Mobile Number (+91 {this.state.mobileNo})</Text>
								</View>

								<View style={styles.otpinput}>
									<TextInput style={[styles.otp, { borderWidth: this.state.aBorder, borderColor: this.state.aBorderColor }]}
										maxLength={1}
										keyboardType="numeric"
										ref={this.a}
										onChangeText={v => this.focusNext(this.a, this.b, v)}></TextInput>
									<TextInput style={[styles.otp, { borderWidth: this.state.bBorder, borderColor: this.state.bBorderColor }]}
										maxLength={1}
										keyboardType="numeric"
										ref={this.b}
										onKeyPress={e => this.focusPrevious(e.nativeEvent.key, this.a)}
										onChangeText={v => this.focusNext(this.b, this.c, v)}></TextInput>
									<TextInput style={[styles.otp, { borderWidth: this.state.cBorder, borderColor: this.state.cBorderColor }]}
										maxLength={1}
										keyboardType="numeric"
										ref={this.c}
										onKeyPress={e => this.focusPrevious(e.nativeEvent.key, this.b)}
										onChangeText={v => this.focusNext(this.c, this.d, v)}></TextInput>
									<TextInput style={[styles.otp, { borderWidth: this.state.dBorder, borderColor: this.state.dBorderColor }]}
										maxLength={1}
										keyboardType="numeric"
										onKeyPress={e => this.focusPrevious(e.nativeEvent.key, this.c)}
										ref={this.d}
										onChangeText={v => this.focusNext(this.d, this.d, v)}></TextInput>
								</View>

								<View style={styles.passwordinput}>
									<Image source={require('../../images/Password.png')} style={styles.textimage}></Image>
